...
extern double* pposX_ghost;
extern double* pposY_ghost;
extern double* pposZ_ghost;
extern double* pwidth_ghost;
...
/////////////////////////// end by Yurong
	...
	if ((ReInfo->_displayMode != RM_DISP_MODE_NONE) && (ReInfo->_displayMode != RM_DISP_MODE_CONSOLE)) {
		if (floor(s->currentTime) == -2.0) {
			ReRaceBigMsgSet("", 1.0);//ReRaceBigMsgSet("Ready", 1.0);
		} else if (floor(s->currentTime) == -1.0) {
			ReRaceBigMsgSet("", 1.0);//ReRaceBigMsgSet("Set", 1.0);
		} else if (floor(s->currentTime) == 0.0) {
			ReRaceBigMsgSet("", 1.0);//ReRaceBigMsgSet("Go", 1.0);
		}
	}
