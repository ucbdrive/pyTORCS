...
#ifdef COLLECTSEG
	std::string acName = acname;
	int dot_idx = 1;
	while (dot_idx <= acName.length() && acname[acName.length() - dot_idx] != '.')
		dot_idx += 1;
	std::string grass_filename = acName.substr(0, acName.length() - dot_idx) + std::string("_grass") + acName.substr(acName.length() - dot_idx, acName.length());
	std::string asphalt_filename = acName.substr(0, acName.length() - dot_idx) + std::string("_asphalt") + acName.substr(acName.length() - dot_idx, acName.length());
	
	replace_grass = grssgLoadAC3D(grass_filename.c_str(), NULL);
	replace_asphalt = grssgLoadAC3D(asphalt_filename.c_str(), NULL);

	temp->addKid(replace_grass);
	temp->addKid(replace_asphalt);
	temp->addKid(desc);
#endif
...
#ifdef COLLECTSEG
    if (drawIndicator == 1) {
    	LandAnchor->replaceKid(desc, replace_grass);
    }
    else if (drawIndicator == 2){
    	LandAnchor->replaceKid(desc, replace_asphalt);
    }
...
#ifdef COLLECTSEG
    if (drawIndicator == 1) {
    	LandAnchor->replaceKid(replace_grass, desc);
    }
    else if (drawIndicator == 2){
    	LandAnchor->replaceKid(replace_asphalt, desc);
    }
    else if (drawIndicator == 3){
		TheScene->addKid(SmokeAnchor);
...
initBackground(void)
{
...
#ifdef COLLECTSEG
    tempBackground = new ssgRoot();
    ssgVtxTable 	*bg_tmp;
    ssgVertexArray	*bg_vtx_tmp;
    ssgTexCoordArray	*bg_tex_tmp;
    ssgColourArray	*bg_clr_tmp;
    ssgNormalArray	*bg_nrm_tmp;
    ssgSimpleState	*bg_st_tmp;
#endif
...
#ifdef COLLECTSEG
	bg_vtx_tmp = new ssgVertexArray(NB_BG_FACES + 1);
	bg_tex_tmp = new ssgTexCoordArray(NB_BG_FACES + 1);
	bg_clr_tmp = new ssgColourArray(1);
	bg_nrm_tmp = new ssgNormalArray(1);
	...
	bg_tmp = new ssgVtxTable(GL_TRIANGLE_STRIP, bg_vtx_tmp, bg_nrm_tmp, bg_tex_tmp, bg_clr_tmp);
	bg_st_tmp = (ssgSimpleState*)grSsgLoadTexState("blue.png");
	bg_st_tmp->disable(GL_LIGHTING);
	bg_tmp->setState(bg_st_tmp);
	bg_tmp->setCullFace(0);
	tempBackground->addKid(bg_tmp);
#endif
